...
from web_checkin import WebCheckinManager  # Исправленный импорт
...
class PharosBotAdvanced:
    def __init__(self):
        ...
        self.web_checkin_manager = WebCheckinManager(self.web3_manager)  # Новый менеджер
    ...
    def get_user_config(self):
        ...
        while True:
            ...
            if swaps in ['y', 'n']:
                ...
                if swaps == 'y':
                    while True:
                        try:
                            swap_count = int(input(f"{Fore.YELLOW + Style.BRIGHT}Number of swaps per round [1-10]: {Style.RESET_ALL}"))
                            if 1 <= swap_count <= 10:
                    ...
                    while True:
                        try:
                            swap_rounds = int(input(f"{Fore.YELLOW + Style.BRIGHT}Number of swap rounds [1-5]: {Style.RESET_ALL}"))
                            if 1 <= swap_rounds <= 5:
                                config['swap_rounds'] = swap_rounds
                                break
                            print(f"{Fore.RED + Style.BRIGHT}Please enter a number between 1 and 5{Style.RESET_ALL}")
                        except ValueError:
                            print(f"{Fore.RED + Style.BRIGHT}Please enter a valid number{Style.RESET_ALL}")
                    
                    while True:
                        try:
                            swap_delay = int(input(f"{Fore.YELLOW + Style.BRIGHT}Delay between swaps (seconds) [5-60]: {Style.RESET_ALL}"))
                            if 5 <= swap_delay <= 60:
                                config['swap_delay'] = swap_delay
                                break
                            print(f"{Fore.RED + Style.BRIGHT}Please enter a number between 5 and 60{Style.RESET_ALL}")
                        except ValueError:
                            print(f"{Fore.RED + Style.BRIGHT}Please enter a valid number{Style.RESET_ALL}")
                break
            print(f"{Fore.RED + Style.BRIGHT}Please enter 'y' or 'n'{Style.RESET_ALL}")
        ...
        while True:
            ...
            if liquidity in ['y', 'n']:
                ...
                if liquidity == 'y':
                    while True:
                        try:
                            liquidity_rounds = int(input(f"{Fore.YELLOW + Style.BRIGHT}Number of liquidity rounds [1-3]: {Style.RESET_ALL}"))
                            if 1 <= liquidity_rounds <= 3:
                                config['liquidity_rounds'] = liquidity_rounds
                                break
                            print(f"{Fore.RED + Style.BRIGHT}Please enter a number between 1 and 3{Style.RESET_ALL}")
                        except ValueError:
                            print(f"{Fore.RED + Style.BRIGHT}Please enter a valid number{Style.RESET_ALL}")
                break
            print(f"{Fore.RED + Style.BRIGHT}Please enter 'y' or 'n'{Style.RESET_ALL}")
            ...
            if tips in ['y', 'n']:
                ...
                if tips == 'y':
                    ...
                    if username:
                        ...
                        while True:
                            try:
                                tip_rounds = int(input(f"{Fore.YELLOW + Style.BRIGHT}Number of tip rounds [1-5]: {Style.RESET_ALL}"))
                                if 1 <= tip_rounds <= 5:
                                    config['tip_rounds'] = tip_rounds
                                    break
                                print(f"{Fore.RED + Style.BRIGHT}Please enter a number between 1 and 5{Style.RESET_ALL}")
                            except ValueError:
                                print(f"{Fore.RED + Style.BRIGHT}Please enter a valid number{Style.RESET_ALL}")
        ...
        while True:
            try:
                wallet_delay = int(input(f"{Fore.YELLOW + Style.BRIGHT}Delay between wallets (seconds) [10-300]: {Style.RESET_ALL}"))
                if 10 <= wallet_delay <= 300:
                    config['wallet_delay'] = wallet_delay
                    break
                print(f"{Fore.RED + Style.BRIGHT}Please enter a number between 10 and 300{Style.RESET_ALL}")
            except ValueError:
                print(f"{Fore.RED + Style.BRIGHT}Please enter a valid number{Style.RESET_ALL}")
        ...
        while True:
            try:
                threads = int(input(f"{Fore.YELLOW + Style.BRIGHT}Number of parallel threads [1-20]: {Style.RESET_ALL}"))
                if 1 <= threads <= 20:
        ...
        try:
            ...
            if config.get('checkin_enabled', False):
                ...
                if web_checkin_result:
                    ...
                    delay = 3 if config.get('threads', 1) > 1 else 5
            ...
            if config.get('swaps_enabled', False):
                ...
                successful_swaps = 0
                max_swaps = config.get('swap_count', 2)
                
                for round_num in range(config.get('swap_rounds', 1)):
                    Logger.log(f"{Fore.MAGENTA + Style.BRIGHT}Swap round {round_num + 1}/{config.get('swap_rounds', 1)}{Style.RESET_ALL}")
                    
                    for i, (from_token, to_token, amount, from_name, to_name) in enumerate(swap_pairs[:max_swaps]):
                        if successful_swaps >= max_swaps * config.get('swap_rounds', 1):
                            break
                            
                        Logger.log(f"{Fore.YELLOW + Style.BRIGHT}Swap {i+1}: {from_name} -> {to_name}{Style.RESET_ALL}")
                        ...
                        if route_data:
                            ...
                            if swap_result:
                                successful_swaps += 1
                                results[f'swap_{round_num+1}_{i+1}'] = swap_result
                            
                            await asyncio.sleep(config.get('swap_delay', 10))
                    
                    if round_num < config.get('swap_rounds', 1) - 1:
            ...
            if config.get('liquidity_enabled', False):
                ...
                for lp_round in range(config.get('liquidity_rounds', 1)):
                    Logger.log(f"{Fore.MAGENTA + Style.BRIGHT}Liquidity round {lp_round + 1}/{config.get('liquidity_rounds', 1)}{Style.RESET_ALL}")
                    ...
                    if usdc_balance >= required_usdc and usdt_balance >= required_usdt:
                        ...
                        if lp_round < config.get('liquidity_rounds', 1) - 1:
                            await asyncio.sleep(30)
                    else:
                        Logger.log(f"{Fore.YELLOW + Style.BRIGHT}Insufficient tokens for liquidity provision. USDC: {usdc_balance:.6f}/{required_usdc:.6f}, USDT: {usdt_balance:.6f}/{required_usdt:.6f}{Style.RESET_ALL}")
                        break
            ...
            if config.get('tips_enabled', False) and config.get('tip_username'):
                ...
                if not contract_ok:
                    ...
                    for tip_round in range(config.get('tip_rounds', 1)):
                        Logger.log(f"{Fore.MAGENTA + Style.BRIGHT}Tip round {tip_round + 1}/{config.get('tip_rounds', 1)}{Style.RESET_ALL}")
                        ...
                        # Обновляем баланс после успешного типа
                        if tip_result:
                            current_balance -= Web3.from_wei(tip_amount, 'ether')
                        
                        if tip_round < config.get('tip_rounds', 1) - 1:
                            Logger.log(f"{Fore.YELLOW + Style.BRIGHT}Waiting 30 seconds before next tip...{Style.RESET_ALL}")
                            await asyncio.sleep(30)
    ...
    async def run_bot(self):
        ...
        try:
            ...
            while True:
                ...
                # Разбиваем кошельки на пакеты для параллельной обработки
                threads = config.get('threads', 1)
                batch_size = max(1, len(private_keys) // threads) if threads > 1 else len(private_keys)
                
                Logger.log(f"{Fore.GREEN + Style.BRIGHT}Processing {len(private_keys)} wallets with {threads} parallel threads{Style.RESET_ALL}")
                Logger.log(f"{Fore.CYAN + Style.BRIGHT}Batch size: {batch_size} wallets per batch{Style.RESET_ALL}")
                
                # Создаем пакеты кошельков
                batches = []
                ...
                for batch_num, batch in enumerate(batches, 1):
                    ...
                    if batch_num < len(batches):
                        batch_delay = config.get('wallet_delay', 30)
                        Logger.log(f"{Fore.YELLOW + Style.BRIGHT}Waiting {batch_delay} seconds before next batch...{Style.RESET_ALL}")
